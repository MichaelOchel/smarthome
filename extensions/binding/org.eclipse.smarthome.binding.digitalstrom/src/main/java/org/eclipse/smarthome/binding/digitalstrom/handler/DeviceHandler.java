                ...
                if (sensorChannlesLoaded()) {
                    ...
                    if (deviceStateUpdate.isBinarayInputType()) {
                        if (deviceStateUpdate.getValueAsShort() == 1) {
                            updateState(getBinaryInputChannelUID(deviceStateUpdate.getTypeAsDeviceBinarayInputEnum()),
                                    OnOffType.ON);
                        } else {
                            updateState(getBinaryInputChannelUID(deviceStateUpdate.getTypeAsDeviceBinarayInputEnum()),
                                    OnOffType.OFF);
                        ...
                        case DeviceStateUpdate.OPEN_CLOSE_ANGLE:
                            if (device.isBlind() && currentChannel != null) {
                                if (deviceStateUpdate.getValueAsInteger() > 0) {
                                    ...
                                    updateState(new ChannelUID(getThing().getUID(), currentChannel), PercentType.ZERO);
                                }
                            }
                            return;
